...
def _kb_lock(kb_path: str) -> "filelock.FileLock":
    """Create a FileLock for a knowledge_base.json file."""
    import filelock

    return filelock.FileLock(kb_path + ".lock", timeout=_KB_LOCK_TIMEOUT)
...
class KnowledgeBase:
    def __init__(
        self, entity_id: str, models_dir: Optional[str], entity_type: str = "project"
    ) -> None:
        ...
        if entity_type == "sector":
            base_dir = (
                os.path.join(models_dir, "sectors") if models_dir else get_sectors_dir()
            )
        else:
            base_dir = (
                os.path.join(models_dir, "projects")
                if models_dir
                else get_projects_dir()
            )
    ...
    def add_entries(self, entries: List[KBEntryDict]) -> int:
        ...
        lock = _kb_lock(self.kb_path)
        with lock:
            # Reload from disk inside the lock to prevent stale reads
            if os.path.exists(self.kb_path):
                with open(self.kb_path, "r", encoding="utf-8") as f:
                    self.entries = json.load(f)

            # Build lookup AFTER reload (must use fresh data)
            ...
            source_rank = {
                "consultant_correction": 2,
                "reclassified_with_guidance": 1,
                "llm_approved": 0,
            }
            ...
            for entry in entries:
                ...
                if desc_norm in existing:
                    ...
                    if new_rank > old_rank or (
                        new_rank == old_rank and classification_changed
                    ):
                        preserved_id = old["id"]
        ...
        if filters:
            ...
            if filters.get("n1"):
                entries = [
                    e
                    for e in entries
                    if e.get("N1", "").lower() == filters["n1"].lower()
                ]
            if filters.get("n4"):
                entries = [
                    e
                    for e in entries
                    if e.get("N4", "").lower() == filters["n4"].lower()
                ]
    ...
    def get_coverage(
        self, hierarchy: Optional[List[HierarchyEntryDict]]
    ) -> KBCoverageDict:
        if not hierarchy:
            return {"total_n4s": 0, "covered": 0, "pct": 0.0, "underserved": []}

        all_n4s = list(
            {entry.get("N4", "").strip() for entry in hierarchy if entry.get("N4")}
        )
        covered_counts = {}
            ...
            entries.append(
                {
                    "description": self._safe_str(
                        row.get(
                            "Descrição",
                            row.get("Descricao", row.get("description", "")),
                        )
                    ),
                    ...
                    "instruction_used": row.get(
                        "Instrução", row.get("Instrucao", row.get("instruction_used"))
                    ),
                }
            )
    ...
    def promote_entries_to(
        self, target_kb: "KnowledgeBase", entry_ids: List[str]
    ) -> int:
...
def merge_kb_entries(
    sector_entries: List[KBEntryDict], project_entries: List[KBEntryDict]
) -> List[KBEntryDict]:
