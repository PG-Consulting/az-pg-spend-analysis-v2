"""
Model Trainer for Spend.AI ML Classifier.
"""
...
def train_model_core(
    ...
    logging.info(
        f"   Removed {removed_categories} categories with < {min_examples} examples"
    )
    logging.info(
        f"   Removed {removed_count} examples ({removed_count / len(df) * 100:.1f}% of data)"
    )
    logging.info(
        f"   Remaining: {len(df_filtered)} examples, {df_filtered['N4'].nunique()} categories"
    )
    ...
    if "Descrição_Normalizada" not in df_filtered.columns:
        ...
        df_filtered["Descrição_Normalizada"] = (
            df_filtered[text_col].astype(str).apply(normalize_text)
        )  # Uses internal normalize function if needed, or from taxonomy_engine
    ...
    logging.info(
        f"4. Splitting data ({int((1 - test_size) * 100)}% train, {int(test_size * 100)}% validation)..."
    )
    ...
    shutil.copy(
        f"{version_dir}/tfidf_vectorizer.pkl",
        f"{sector_model_dir}/tfidf_vectorizer.pkl",
    )
    shutil.copy(f"{version_dir}/classifier.pkl", f"{sector_model_dir}/classifier.pkl")
    shutil.copy(
        f"{version_dir}/label_encoder.pkl", f"{sector_model_dir}/label_encoder.pkl"
    )
    ...
    if all(col in df.columns for col in hierarchy_cols):
        ...
        logging.info(
            f"   Saved hierarchy for {len(hierarchy)} N4 categories (+ versioned copy)"
        )
    else:
        logging.warning(
            "   N1/N2/N3 columns not found in training data. Hierarchy not updated."
        )
    ...
    if len(history) > MAX_VERSIONS:
        ...
        logging.info(
            f"Cleanup: Removing {len(versions_to_remove)} old versions (Max allowed: {MAX_VERSIONS})"
        )
...
def train_model(sector: str, dataset_path: str, models_dir: str = "models") -> dict:
    ...
    result = train_model_core(
        df=df, sector=sector, models_dir=models_dir, training_filename=training_filename
    )
