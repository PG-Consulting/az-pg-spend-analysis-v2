...
def reclassify_items_endpoint(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return func.HttpResponse(safe_json_dumps({"results": results}),
                                 status_code=200, mimetype="application/json")
    except Exception as e:
        logger.error(f"ReclassifyItems error: {e}", exc_info=True)
...
def approve_classifications_endpoint(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if not os.path.isdir(job_dir):
            return func.HttpResponse(json.dumps({"error": "Job not found"}),
        ...
        return func.HttpResponse(safe_json_dumps({
            "success": True,
            "kb_added": kb_added,
            "summary": {
                "total": len(decisions),
        ...
        logger.error(f"ApproveClassifications error: {e}", exc_info=True)
        return func.HttpResponse(json.dumps({"error": str(e)}), status_code=500, mimetype="application/json")
